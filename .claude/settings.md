...
1. **JavaScript/TypeScript Formatting**: Auto-format with Prettier (30s timeout)
   - **Triggers**: After editing `.js`, `.jsx`, `.ts`, `.tsx` files
   - **Command**: `npx prettier --write`

2. **Rust Formatting**: Auto-format with cargo fmt (30s timeout)
   - **Triggers**: After editing `.rs` files
   - **Command**: `cargo fmt --all`

3. **Rust Linting**: Run clippy on changes (60s timeout, non-blocking)
   - **Triggers**: After editing `.rs` files
   - **Command**: `cargo clippy --all-targets --all-features`
   - **Behavior**: Shows errors but doesn't block

4. **WASM Rebuild Reminder**: Alert when renderer changes (5s timeout)
   - **Triggers**: After editing `crates/klyric-renderer/` files
   - **Behavior**: Reminds to run `npm run build:wasm`

5. **NPM Install**: Auto-install after package.json changes (60s timeout)
   - **Triggers**: After editing `package.json` files
   - **Command**: `npm install`

6. **Test Runner**: Run tests after test file changes (90s timeout)
