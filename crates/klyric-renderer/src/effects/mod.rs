...
use crate::model::modifiers::{AppearMode, EffectLayer, Modifier, ScopeType, Selector};
...
impl EffectEngine {
    ...
    pub fn apply_specific_layers_to_render(
        ...
        for &idx in indices {
            ...
            if Self::matches_selector(&layer.selector, ctx) {
                for modifier in &layer.modifiers {
                    Self::apply_modifier_to_render(&mut transform, modifier, current_time, ctx);
                }
            }
        }
        transform
    ...
    fn apply_modifier_to_render(
        transform: &mut RenderTransform,
        modifier: &Modifier,
        time: f64,
        _ctx: &TriggerContext,
    ) {
...
mod tests_global_detection {
    ...
    fn test_global_allowlist_selectors() {
        ...
        assert!(EffectEngine::is_layer_global(&make_layer(
            Selector::All,
            safe_mods.clone()
        )));
        assert!(EffectEngine::is_layer_global(&make_layer(
    ...
    fn test_global_blocklist_selectors() {
        ...
        assert!(!EffectEngine::is_layer_global(&make_layer(
            Selector::Pattern { n: 2, offset: 0 },
            safe_mods.clone()
        )));

        // Word/Syllable excluded for safety
        assert!(!EffectEngine::is_layer_global(&make_layer(
            Selector::Scope(ScopeType::Word),
            safe_mods.clone()
        )));
        assert!(!EffectEngine::is_layer_global(&make_layer(
            Selector::Scope(ScopeType::Syllable),
            safe_mods.clone()
        )));
