...
pub fn parse_document(json: &str) -> Result<KLyricDocumentV2> {
    let doc: KLyricDocumentV2 = serde_json::from_str(json)
        .context("Failed to parse KLyric v2.0 JSON")?;
        
    // Validate version
    if doc.version != "2.0" {
        return Err(anyhow::anyhow!("Unsupported KLyric version: {}", doc.version));
    }
...
mod tests {
    ...
    fn test_parser_validates_version() {
        ...
        assert!(result.unwrap_err().to_string().contains("Unsupported KLyric version"));
    }
}
