use super::model::{KLyricDocumentV2, Style};
...
mod tests {
    use super::*;
    use crate::model::{FillStroke, Font, Glow, Project, Resolution, Shadow, StateColors, Stroke};
    ...
    fn test_merge_partial() {
        ...
        assert!(resolved.font.is_some());
        assert_eq!(
            resolved.font.as_ref().unwrap().family,
            Some("BaseFont".to_string())
        );
