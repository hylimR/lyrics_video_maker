...
use skia_safe::{gpu::{gl::Interface, DirectContext, SurfaceOrigin, Protected}, ColorType, Surface};
...
fn main() -> Result<()> {
    ...
    let not_current_gl_context = unsafe {
        gl_display.create_context(&gl_config, &context_attributes).map_err(|e| anyhow::anyhow!(e.to_string()))?
    };
    ...
    let gl_surface = unsafe {
        gl_display.create_window_surface(&gl_config, &surface_attributes).map_err(|e| anyhow::anyhow!(e.to_string()))?
    };

    let gl_context = not_current_gl_context.make_current(&gl_surface).map_err(|e| anyhow::anyhow!(e.to_string()))?;
    ...
    let mut gr_context = DirectContext::new_gl(interface, None).expect("Failed to create Skia Context");
    
    let mut surface = create_skia_surface(
        &mut gr_context, 
        attrs.width, 
        attrs.height
    );
